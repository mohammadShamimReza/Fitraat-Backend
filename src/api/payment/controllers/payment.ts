...
module.exports = {
  ...
  async init(ctx) {
    ...
    const data = {
      ...
      success_url: `http://10.0.2.2:1337/api/payment/success/${userId}`,
    ...
    try {
      ...
      if (result) {
        ctx.response.redirect(
          `http://localhost:3000/payment/redirectSuccess/${userId}`
        );
      } else {
        ctx.response.redirect(`http://localhost:3000/contant`);
      }
    } catch (error) {
